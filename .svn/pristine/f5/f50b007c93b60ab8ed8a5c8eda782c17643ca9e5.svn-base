...
public class HeadMypageServiceImpl implements IHeadMypageService{
	...
	public ServiceResult updateMypage(EmployeeVO employeeVO) {
		...
		if(employeeVO.getAttachfileList() != null) {
			...
			cnt += headMyDAO.updateNoImgEmpMyloyee(employeeVO);
		}
	...
	public vmemberVO selectSe(String id) {
		// TODO Auto-generated method stub
		return headMyDAO.selectSe(id);
	}
	@Override
	public headVO selectHeadMy(String mem_id) {
		// TODO Auto-generated method stub
		return headMyDAO.selectHeadMy(mem_id);
	}
	@Transactional
	@Override
	public ServiceResult updateheadMypage(headVO headVO) {
		...
		logger.info("===========employeeVO : {}============", headVO);
		if(headVO.getAttachfileList() != null) {
			
			if(headVO.getBefore_att_fileno()!=null && headVO.getBefore_att_fileno().length()>0) {
				logger.info("===========employeeVO.getDelAttSeq() : {}============", headVO.getBefore_att_fileno());
				logger.info("===========employeeVO.getAttachfileList() if문 : {}============", headVO.getAttachfileList());
				cnt += deleteFileProcesses2(headVO);
			}
			
			cnt += headMyDAO.updateheadMypage(headVO);
		}else {
			cnt += headMyDAO.updateheadNoImgMypage(headVO);
		}
	
		folder = new File(attatchPath+"/"+headVO.getHead_code()+"/"+headVO.getHead_id()+"/emp/file");
		saveFolder = folder.getAbsolutePath();
		
		folder_BL = new File(attatchPath + "/" + headVO.getHead_code()+subPath);
		saveFolder_BL = folder_BL.getAbsolutePath();
	...
	private int deleteFileProcesses2(headVO employeeVO) {
	...
	private int processes2(headVO employeeVO) {
		...
		if(attatchList != null && attatchList.size() > 0) {
			...
			cnt += headMyDAO.insertheadMyImage(employeeVO);
			try {
				for(int i=0; i<attatchList.size(); i++) {
					AttachfileVO attatch = attatchList.get(i);
					if(i==0) {
