...
<body>
    <div id="app">
						...
						<div class="rowRight">
							<nav aria-label="breadcrumb" >
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page">문의 사항</li>
								</ol>
							</nav>
						</div>
					</div>
										...
										<c:choose>
											<c:when test="${not empty pagingVO.dataList }">
												<c:forEach items="${pagingVO.dataList }" var="question">
													...
													<%-- 	<c:if test="${datalist.ques_sttus_code eq '답변안한글'}">
															<c:url value="/fran/question/answerList" var="viewURL">
																<c:param name="what" value="${question.ques_seq}" />
															</c:url>
														<td><a href="${viewURL}">답변완료</td>
														</c:if>
														<td><a href="${viewURL}">답변준비중</td> --%>
        ...
        <!-- 검색 폼 -->
	    <form id="searchForm" action="${cPath }/head/question" >
	    ...
	    <script type="text/javascript">
	      ...
	      /*   //검색
	    	function fn_search(obj){
	    		...
	    		console.log(searchForm.find("input[name="+name+"]").val(obj.value));
	       ...
	       searchForm = $("#searchForm").ajaxForm({
	            ...
	            success: function(resp) {
	                ...
	                if (resp.dataList) {
	                   ...
	                   /*  	let answer
	                    	if(datalist.ques_sttus_code=="답변안한글"){
	                    		answer= "<a href='${viewURL}'>답변완료";
// 	                    		$(".ama").html(answer);
	                    		
	                    		console.log(answer)
                        	}else{
                        		answer="<a href='${viewURL}'>답변준비중";
                        	} */
	                        ...
	                        let tr = $("<tr>").append(
	                        	...
	                        	$("<td>").addClass("center").html(datalist.ques_sttus_code)
	       ...
	       $(listBody).on("click",'tr',function(){
	        	...
	        	location.href ="${cPath }/head/question/detail?what="+ ques_seq;
	        })
