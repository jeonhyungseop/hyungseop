...
public class EmailServiceImpl implements IEmailService  {
	...
	public void sendMail(HeadAnswerVO dto) {
		 try {
	            ...
	            msg.addRecipient(RecipientType.TO, new InternetAddress(dto.getQues_email()));
	            ...
	            // 이메일 발신자
	            msg.addFrom(new InternetAddress[] { new InternetAddress(dto.getAdmin_email(), dto.getAnswer_writer_id()) });
	 
	            // 이메일 제목 (인코딩을 해야 한글이 깨지지 않음)
	            msg.setSubject(dto.getAnswer_title(), "utf-8");
	            // 이메일 본문 (인코딩을 해야 한글이 깨지지 않음)
	            msg.setText(dto.getAnswer_cn(), "utf-8");
