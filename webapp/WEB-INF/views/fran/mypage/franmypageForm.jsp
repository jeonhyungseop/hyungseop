		...
		<div class="card">
					...
					<div class="rowRight">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">가맹점수정</li>
							</ol>
						</nav>
					</div>
				</div>
			...
			<div class="card-body">
				<form:form method="post" enctype="multipart/form-data"
					class="empForm" action="${cPath}/fran/mypage/modify">
					...
					<table class="table table-hover">
								...
								<div class="image-container ">
									<c:choose>
										<c:when test="${employeeOne.att_fileno eq null}">
											...
											<img style="width: 350px;" id="preview-image"
												src="${cPath}/resources/images/logo/profileIMG.jpg">
										</c:when>
										<c:otherwise>
											<c:forEach items="${employeeOne.attachfileList}" var="attach"
												varStatus="status">
												<c:if test="${status.index eq 0}">
													<img style="width: 250px;" id="preview-image"
														class="myImage" alt=""
														src="data:image/jpeg;base64,${attach.file_img}">
												</c:if>
											</c:forEach>
										</c:otherwise>
									</c:choose>
									...
									<input class="form-control photo" name="emp_files"
										style="display: block;" type="file" id="input-image">
								</div> <!-- <input class="form-control photo" type="file" name="emp_files"> -->
								<input type=hidden name="att_fileno"
								value="${employeeOne.att_fileno }">
						...
						<tr>
							...
							<td><input class="form-control formWidth nameCheck"
								type="text" name="fowner_nm" value="${employeeOne.fowner_nm }">
								<input type="hidden" name="fran_id"
								value="${employeeOne.fran_id }"> <span
								class="namespan checkspan"></span></td>
						</tr>

						<tr>
							<th><span class="nes"></span> 우편번호</th>
							<td><input type="text" class="form-control postnum"
								id="postcode" placeholder="우편번호" name="fran_zipno"
								value="${employeeOne.fran_zipno}" /> <input type="button"
								onclick="DaumPostcode()" value="우편번호 찾기" class="btn btn-primary" />
								<span class="postspan checkspan"></span></td>
						</tr>
						<tr>
							<th><span class="nes"></span> 주소</th>
							<td><input type="text" class="form-control" id="address"
								placeholder="주소" name="fran_baadd"
								value="${employeeOne.fran_baadd}" /> <input type="text"
								class="form-control" id="detailAddress" placeholder="상세주소"
								name="fran_deadd" value="${employeeOne.fran_deadd}" /> <span
								class="addressspan checkspan"></span></td>
						</tr>
						<tr>
							<th><span class="nes"></span>이메일</th>
							<td><input class="form-control emailCheck formWidth"
								type="text" name="fowner_email"
								value="${employeeOne.fowner_email }"> <span
								class="emailspan checkspan"></span></td>
						</tr>
						<tr>
							<th><span class="nones"></span>전화번호</th>
							<td><input class="form-control telCheck formWidth"
								type="text" name="fran_telno" placeholder="- 제외하고 입력"
								value="${employeeOne.fran_telno }"> <span
								class="telspan checkspan"></span></td>
						</tr>
					...
					<a href="${cPath}/fran/mypage" class="btn btn-secondary floatRight"><i
						class="bi bi-arrow-clockwise"></i>돌아가기</a>
				</form:form>
				<!-- card-body 끝 -->
			</div>
			<!-- card 끝 -->
...
<script type="text/javascript">
 ...
 $(function(){
	...
	//이름 체크
	 ...
	 //상호명
	  $('.nameCheck2').on('keyup', function(){
		 
		 let nameCheck2 = $('.nameCheck2').val();
		 if(nameCheck2.length < 3 || 5 < nameCheck2.length){
			 $('.namespan2').text('3~5자리로 입력해주세요');
			 return;
		 ...
		 if(regname.test(nameCheck2)){

		 }else{
			$('.namespan2').text("올바른 형식이 아닙니다!");
		} 
