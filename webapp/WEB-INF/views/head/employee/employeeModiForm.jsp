...
<div id="app">
    	<section class="section">
    		...
    		<div class="card">
				...
				<!-- card-body 영역 -->
					<div class="card-body">
					<form:form  method="post" enctype="multipart/form-data" class="empForm" action="${cPath}/head/employee/modify">
									...
									<c:otherwise>
										<c:forEach items="${employeeOne.attachfileList}" var="attach" varStatus="status">
											<c:if test="${status.index eq 0}">
												<img style="width: 255px;" id="preview-image" class="myImage" alt="" src="data:image/jpeg;base64,${attach.file_img}">
											</c:if>
										</c:forEach>
									</c:otherwise>
								</c:choose>
					...
					<!-- 	<tr>
							...
							<td><input class="form-control formWidth pwdCheck" type="password" name="emp_pass" placeholder="영문 대소문자,숫자 ,특수문자 포함  8~12자리 입력">
						...
						<tr>
							<th><span class="nones">*</span>직급<input type="hidden" class="hiddenClsfCode" value="${employeeOne.emp_clsf_code }"></th>				
							<td>
								<select class="form-control formWidth" name="emp_clsf_code">
									...
									<c:if test="${not empty comcodeECL }">
	                               		<c:forEach items="${comcodeECL }" var="ecl">
	                               			<option value="${ecl.com_code }" class="comcodeEach">${ecl.com_code_nm }</option>
	                               		</c:forEach>
	                                 </c:if>
                                 </select>
							</td>				
						...
						<tr>
							<th><span class="nones">*</span>부서<input class="hiddenDeptSeq" type="hidden" value="${employeeOne.deptList[0].dept_seq}"></th>				
							<td>
								<select class="form-control formWidth" name="dept_seq" class="deptSelect">
									<option value="">부서선택</option>
									<c:if test="${not empty deptList }">
	                               		<c:forEach items="${deptList }" var="dept">
	                               			<option value="${dept.dept_seq }" class="dept${dept.dept_seq}">${dept.dept_nm}</option>
	                               		</c:forEach>
	                                 </c:if>
                                 </select>
							</td>				
						...
						<tr>
							<th><span class="nes">*</span>직원구분<input type="hidden" value="${employeeOne.emp_se_code}" class="hiddenSeCode"></th>				
							<td>
								<select class="form-control formWidth empSeCheck" name="emp_se_code">
									...
									<c:if test="${not empty comcodeESE }">
	                               		<c:forEach items="${comcodeESE }" var="ese">
	                               			<option value="${ese.com_code }" class="seCodeEach">${ese.com_code_nm }</option>
	                               		</c:forEach>
	                                 </c:if>
								</select>
						...
						<tr>
							<th><span class="nes">*</span>계좌번호<input type="hidden" value="${employeeOne.emp_bnm }" class="hiddenBNM"></th>				
							<td>
								<select class="form-control bankSelect" name="emp_bnm">
									<option class="bacnkSelect" value="">은행선택</option>
									<option class="bacnkSelect" value="KB국민은행">KB국민은행</option>
									<option class="bacnkSelect" value="KEB하나은행">KEB하나은행</option>
									<option class="bacnkSelect" value="신한은행">신한은행</option>
									<option class="bacnkSelect" value="우리은행">우리은행</option>
									<option class="bacnkSelect" value="NH농협">NH농협</option>
									<option class="bacnkSelect" value="IBK기업은행">IBK기업은행</option>
									<option class="bacnkSelect" value="KDB산업은행">KDB산업은행</option>
									<option class="bacnkSelect" value="한국씨티은행">한국씨티은행</option>
								</select>
 ...
 $(function(){
	//검증
	//val($(".hiddenDeptSeq").val())
	//select옵션 선택 - 부서선택
	$(".dept"+$(".hiddenDeptSeq").val()).prop("selected", true);
	
	//직급구분
	$('.comcodeEach').each(function(idx, com){
		
		if($(this).text() == $('.hiddenClsfCode').val()){
	...
	//직원구분	
	$('.seCodeEach').each(function(idx, com){
		
		if($(this).text() == $('.hiddenSeCode').val()){
			$(this).prop("selected", true);
	...
	//은행명 
	$('.bacnkSelect').each(function(idx, com){
		
		if($(this).text() == $('.hiddenBNM').val()){
			$(this).prop("selected", true);
